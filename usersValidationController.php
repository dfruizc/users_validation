...
class UsersValidationController
{
    ...
    public function usersValidation()
    {
        ...
        $usersToValidate = $userModel->getusers('2', 'U', "AND \"LMS_ID\" IS NOT NULL");

        if (!is_array($usersToValidate) || count($usersToValidate) < 1) {
            echo "No hay usuarios con LMS_ESTADO = 2, OPERATION = 'U', y LMS_ID IS NOT NULL para validar.\n";
            return;
        }
        ...
        foreach ($usersToValidate as $user) {
            ...
            $username = $user['USR_NUM_DOC_CONCAT'];
            $lmsid = $user['LMS_ID'];
            ...
            if (!empty($response) && isset($response[0]['username'], $response[0]['id'])) {
                $userFromZajuna = $response[0];
                $zajunaUserId = $userFromZajuna['id'];
                $zajunaUsername = $userFromZajuna['username'];

                if ($zajunaUserId == $lmsid && $zajunaUsername == $username) {
                    // Actualizar el estado a P si coincide
                    $userModel->updateException($username, 'USUARIO_LMS', 2);
                    echo "Usuario $username validado correctamente. Estado actualizado a P.\n";
                    ...
                    echo "Usuario $username con LMS_ID $zajunaUserId no coincide. Estado actualizado a 6767.\n";
                ...
                echo "validación para Usuario $username no encontrado en Zajuna. Reportado en LOG.HISTORIES.\n";
                logModel::msj_exception("validación para Usuario $username no encontrado en Zajuna.", 'error', 'usersValidation');
            }
