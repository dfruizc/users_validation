...
{
    global $courseModel;  // Usar el modelo en esta función
    ...
    foreach ($registers as $register) {
        ...
        // Validar que semilla_id no sea NULL
        if (is_null($semilla_id) || $semilla_id === '') {
            echo "El valor de semilla_id es NULL o vacío para este registro.\n";
            continue; // Saltar este registro si semilla_id no es válido
        }

            // Insertar en la base de datos
            $courseModel->insert_course_content(
                $semilla_id, 
                $fic_id, 
                $curso_id, 
                $type, 
                ...
                count(get_course_contents($semilla_id)),  // Cantidad de secciones en semilla
                count(get_course_contents($curso_id))     // Cantidad de secciones en la ficha
            );
